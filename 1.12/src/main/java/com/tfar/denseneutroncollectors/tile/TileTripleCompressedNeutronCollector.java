package com.tfar.denseneutroncollectors.tile;
...
public class TileTripleCompressedNeutronCollector extends TileMachineBase implements IInventory{

    public static int ticks = production_ticks;

    private ItemStack neutrons = ItemStack.EMPTY;
    private int progress;
    private Item item = ForgeRegistries.ITEMS.getValue(new ResourceLocation("avaritia:block_resource"));
    private ItemStack stack = new ItemStack(item, 1, 0);

    @Override
    public void doWork() {
        if (++progress >= ticks) {
            if (neutrons.isEmpty()) {
                neutrons = ItemUtils.copyStack(stack, 1);
            } else if (ItemUtils.areStacksSameType(neutrons, stack)) {
    ...
    public String getName() {
        return "container.double_compressed_neutron";
    }
