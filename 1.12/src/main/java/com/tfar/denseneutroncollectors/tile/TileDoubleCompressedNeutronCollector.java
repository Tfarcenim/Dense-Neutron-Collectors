...
public class TileDoubleCompressedNeutronCollector extends TileMachineBase implements IInventory{
    ...
    public void doWork() {
        if (++progress >= ticks) {
            if (neutrons.isEmpty()) {
                neutrons = ItemUtils.copyStack(ModItems.neutronium_ingot, 1);
            } else if (ItemUtils.areStacksSameType(neutrons, ModItems.neutronium_ingot)) {
