...
public class BlockTripleCompressedNeutronCollector extends BlockContainer implements IModelRegister {

    public BlockTripleCompressedNeutronCollector() {
        super(Material.IRON);
        setSoundType(SoundType.METAL);
        setHardness(20);
        setUnlocalizedName("denseneutroncollectors:triple_compressed_neutron_collector");
        setHarvestLevel("pickaxe", 3);
        setCreativeTab(Avaritia.tab);
        setRegistryName("triple_compressed_neutron_collector");
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        if (world.isRemote) {
            ...
            player.openGui(DenseNeutronCollectors.instance, 2, world, pos.getX(), pos.getY(), pos.getZ());
            return true;
        }
    }
    ...
    public TileEntity createNewTileEntity(World world, int meta) {
        return new TileTripleCompressedNeutronCollector();
    }

    @Override
    public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase player, ItemStack stack) {
        TileEntity tile = world.getTileEntity(pos);
        if (tile instanceof TileTripleCompressedNeutronCollector) {
            TileTripleCompressedNeutronCollector machine = (TileTripleCompressedNeutronCollector) tile;
    ...
    public void breakBlock(World world, BlockPos pos, IBlockState state) {
        TileTripleCompressedNeutronCollector collector = (TileTripleCompressedNeutronCollector) world.getTileEntity(pos);
    ...
    public void registerModels() {
        ...
        ModelLoader.setCustomStateMapper(this, new StateMapperBase() {
            ...
            protected ModelResourceLocation getModelResourceLocation(IBlockState state) {
                String modelLoc = "type=triple_compressed_neutron_collector";
        ...
        ModelResourceLocation invLoc = new ModelResourceLocation(location, "type=triple_compressed_neutron_collector,facing=north,active=true");
