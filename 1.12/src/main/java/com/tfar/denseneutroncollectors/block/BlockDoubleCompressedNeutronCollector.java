...
public class BlockDoubleCompressedNeutronCollector extends BlockContainer implements IModelRegister {

    public BlockDoubleCompressedNeutronCollector() {
        super(Material.IRON);
        setSoundType(SoundType.METAL);
        setHardness(20);
        setUnlocalizedName("denseneutroncollectors:double_compressed_neutron_collector");
        setHarvestLevel("pickaxe", 3);
        setCreativeTab(Avaritia.tab);
        setRegistryName("double_compressed_neutron_collector");
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        if (world.isRemote) {
            ...
            player.openGui(DenseNeutronCollectors.instance, 1, world, pos.getX(), pos.getY(), pos.getZ());
            return true;
        }
    }
    ...
    public TileEntity createNewTileEntity(World world, int meta) {
        return new TileDoubleCompressedNeutronCollector();
    }

    @Override
    public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase player, ItemStack stack) {
        TileEntity tile = world.getTileEntity(pos);
        if (tile instanceof TileDoubleCompressedNeutronCollector) {
            TileDoubleCompressedNeutronCollector machine = (TileDoubleCompressedNeutronCollector) tile;
    ...
    public void breakBlock(World world, BlockPos pos, IBlockState state) {
        TileDoubleCompressedNeutronCollector collector = (TileDoubleCompressedNeutronCollector) world.getTileEntity(pos);
    ...
    public void registerModels() {
        ResourceLocation location = new ResourceLocation("denseneutroncollectors:machine");
        ModelLoader.setCustomStateMapper(this, new StateMapperBase() {
            @Override
            protected ModelResourceLocation getModelResourceLocation(IBlockState state) {
                String modelLoc = "type=double_compressed_neutron_collector";
        ...
        ModelResourceLocation invLoc = new ModelResourceLocation(location, "type=double_compressed_neutron_collector,facing=north,active=true");
